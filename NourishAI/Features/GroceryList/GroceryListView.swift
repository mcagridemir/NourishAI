// NourishAI — GroceryListView.swift
import SwiftUI
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Text("\(Int(vm.progress * 100))% done")
                    .font(NourishTheme.Font.caption()).foregroundStyle(NourishTheme.Color.primary)
            }
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Rectangle().fill(NourishTheme.Color.primary)
                        ...
                        .animation(NourishTheme.Animation.smooth, value: vm.progress)
                }
            }
            .frame(height: 3)
            Divider()
            ...
            List {
                ForEach($vm.sections, id: \.category) { $section in
                    Section {
                        ...
                        HStack {
                            Image(systemName: section.icon).foregroundStyle(NourishTheme.Color.primary)
    ...
    var body: some View {
        HStack {
            Image(systemName: item.isChecked ? "checkmark.circle.fill" : "circle")
                .foregroundStyle(item.isChecked ? NourishTheme.Color.primary : .secondary)
                .font(.title3)
...
private struct EmptyGroceryView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            Image(systemName: "cart.badge.plus").font(.system(size: 60)).foregroundStyle(NourishTheme.Color.primaryLight)
            Text("No grocery list yet").font(NourishTheme.Font.headline(22))
            Text("Generate a smart shopping list from your active meal plan.").font(NourishTheme.Font.body()).foregroundStyle(.secondary).multilineTextAlignment(.center).padding(.horizontal)
