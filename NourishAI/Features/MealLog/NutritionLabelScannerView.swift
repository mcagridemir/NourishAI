// NourishAI — NutritionLabelScannerView.swift
...
struct IngredientHealthEngine {
    ...
    static func explanation(for ingredient: String) -> String? {
        ...
        if lower.contains("palm oil") || lower.contains("palm kernel") {
            return "High in saturated fat; linked to cardiovascular risk and environmental concerns."
        }
        if lower.contains("hydrogenated") {
            return "Contains trans fats, which raise LDL cholesterol."
        }
        if lower.contains("high fructose corn syrup") || lower.contains("glucose-fructose") || lower.contains("glucose fructose") {
            return "Rapidly spikes blood sugar; associated with obesity and metabolic syndrome."
        }
        if lower.contains("glucose syrup") || lower.contains("corn syrup") {
            return "Concentrated sugar source; contributes to excess calorie intake."
        }
        if lower.contains("sodium nitrate") || lower.contains("sodium nitrite") {
            return "Preservatives in processed meats linked to increased cancer risk."
        }
        if lower.contains("carrageenan") {
            return "Controversial additive; may cause digestive inflammation in some people."
        }
        if lower.contains("bha") || lower.contains("bht") || lower.contains("tbhq") {
            return "Synthetic antioxidant preservatives; possible carcinogen at high doses."
        }
        if lower.contains("artificial") {
            return "Synthetic flavour/colour with no nutritional benefit."
        }
        if lower.contains("aspartame") || lower.contains("acesulfame") || lower.contains("saccharin") {
            return "Artificial sweetener; some studies link them to altered gut microbiome."
        }
        return nil
    }
}
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 16) {
                    ForEach([0.5, 1.0, 1.5, 2.0], id: \.self) { mult in
                        Button {
                            withAnimation(NourishTheme.Animation.snappy) { result.servingsUsed = mult }
                        } label: {
                            Text(mult == 0.5 ? "½" : mult == 1.0 ? "1" : mult == 1.5 ? "1½" : "2")
                                .font(NourishTheme.Font.headline())
                                .frame(maxWidth: .infinity)
                                .padding(.vertical, 10)
                                .background(result.servingsUsed == mult ? NourishTheme.Color.primary : NourishTheme.Color.surface)
    ...
    private func macroCell(label: String, value: String, unit: String, color: Color) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(alignment: .firstTextBaseline, spacing: 3) {
                Text(value).font(NourishTheme.Font.headline(18)).foregroundStyle(color)
...
private struct IngredientsAnalysisView: View {
    ...
    private var riskLabel: String {
        switch overallRisk {
        case .high:     return "Contains concerning ingredients"
        case .moderate: return "Some ingredients to watch"
        case .none:     return "Clean ingredient list"
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                ...
                HStack {
                    Text(isExpanded ? "Hide full list" : "Show all \(ingredients.count) ingredients")
                        .font(NourishTheme.Font.caption(12))
                        .foregroundStyle(NourishTheme.Color.primary)
...
private struct FlaggedIngredientRow: View {
    ...
    private var label: String { risk == .high ? "High concern" : "Watch" }

    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Button {
                withAnimation(NourishTheme.Animation.snappy) { showingExplanation.toggle() }
            } label: {
                HStack(spacing: 8) {
                    Image(systemName: icon)
                        .foregroundStyle(color)
