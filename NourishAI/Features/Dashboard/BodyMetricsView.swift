// NourishAI — BodyMetricsView.swift
...
struct BodyMetricsView: View {
    ...
    private var bmiCategory: (label: String, color: Color) {
        ...
        case 18.5..<25: return ("Healthy weight", NourishTheme.Color.primary)
                ...
                VStack(spacing: NourishTheme.Spacing.lg) {
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                        ...
                        MetricCard(label: "TDEE", value: "\(Int(tdee))", unit: "kcal/day",
                                   icon: "figure.walk", color: NourishTheme.Color.primary,
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack(spacing: 0) {
                            VStack(spacing: 4) {
                                ...
                                Text("Lower").font(NourishTheme.Font.caption(11)).foregroundStyle(.secondary)
                            }
                            ...
                            VStack(spacing: 4) {
                                Text(String(format: "%.1f kg", idealWeightRange.max))
                                    .font(NourishTheme.Font.headline(14))
                                Text("Upper").font(NourishTheme.Font.caption(11)).foregroundStyle(.secondary)
                            }
                            .frame(maxWidth: .infinity)
                        }
                        ...
                        ForEach(ActivityLevel.allCases, id: \.self) { level in
                            HStack(spacing: 10) {
                                Image(systemName: user.activityLevel == level ? "circle.fill" : "circle")
                                    .foregroundStyle(user.activityLevel == level ? NourishTheme.Color.primary : .secondary)
                                    .font(.system(size: 12))
        ...
        VStack(spacing: 16) {
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    // Background gradient
                    LinearGradient(
                        colors: [.blue, NourishTheme.Color.primary, .yellow, .orange, .red],
            ...
            // Scale labels
            HStack {
                Text("15").font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
                Spacer()
                Text("18.5").font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
                Spacer()
                Text("25").font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
                Spacer()
                Text("30").font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
                Spacer()
                Text("40+").font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
            }
        }
        .padding()
        .nourishCard()
...
private struct MetricCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(note).font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary.opacity(0.7))
        }
