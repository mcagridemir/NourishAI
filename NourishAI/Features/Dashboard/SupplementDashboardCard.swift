// NourishAI — SupplementDashboardCard.swift
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text("\(doneCount)/\(active.count) taken")
                    ...
                    .foregroundStyle(doneCount == active.count ? NourishTheme.Color.primary : .secondary)
            }
            ...
            HStack(spacing: 8) {
                ForEach(shown) { supp in
                    Button {
                        ...
                        .background((Color(hex: supp.color) ?? NourishTheme.Color.primary).opacity(supp.isLoggedToday ? 0.15 : 0.06))
                        .clipShape(Capsule())
                    }
                    .buttonStyle(.plain)
                }
