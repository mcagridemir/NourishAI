// NourishAI — CaloriePredictionCard.swift
...
struct CaloriePredictionCard: View {
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text("\(predictedTotal)")
                    ...
                    .foregroundStyle(isOverBudget ? .orange : NourishTheme.Color.primary)
                Text("kcal predicted")
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    // Eaten portion
                    RoundedRectangle(cornerRadius: 4)
                        .fill(NourishTheme.Color.primary)
                    ...
                    RoundedRectangle(cornerRadius: 4)
                        .fill(isOverBudget ? Color.orange.opacity(0.5) : NourishTheme.Color.primary.opacity(0.3))
                        .frame(width: geo.size.width * predFrac, height: 8)
                        .offset(x: predStart)
                }
            }
            .frame(height: 8)
            .animation(NourishTheme.Animation.smooth, value: predictedTotal)
    ...
    private var forecastBadge: some View {
        let (label, color): (String, Color) = isOnTrack
            ? ("On track", NourishTheme.Color.primary)
            : isOverBudget
                ? ("Over budget", .orange)
                : ("Under budget", .blue)
        return Text(label)
    ...
    private func predStat(label: String, value: String, unit: String, color: Color) -> some View {
        VStack(spacing: 2) {
            HStack(alignment: .firstTextBaseline, spacing: 2) {
                Text(value).font(NourishTheme.Font.headline(16)).foregroundStyle(color)
                Text(unit).font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
            }
            Text(label).font(NourishTheme.Font.caption(10)).foregroundStyle(.secondary)
        }
