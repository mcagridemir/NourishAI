// NourishAI — SupplementTrackerView.swift
...
struct SupplementTrackerView: View {
    ...
    private var progressHeader: some View {
        HStack(spacing: NourishTheme.Spacing.lg) {
            ZStack {
                Circle()
                    .stroke(NourishTheme.Color.primary.opacity(0.15), lineWidth: 10)
                    .frame(width: 80, height: 80)
                Circle()
                    .trim(from: 0, to: active.isEmpty ? 0 : Double(todayDoneCount) / Double(active.count))
                    .stroke(NourishTheme.Color.primary,
                            style: StrokeStyle(lineWidth: 10, lineCap: .round))
                    .rotationEffect(.degrees(-90))
                    .frame(width: 80, height: 80)
                    .animation(NourishTheme.Animation.smooth, value: todayDoneCount)
                VStack(spacing: 0) {
                    Text("\(todayDoneCount)")
        ...
        .padding()
        .background(NourishTheme.Color.primary.opacity(0.06))
        ...
        HStack(spacing: 14) {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 12)
                    .fill((Color(hex: supp.color) ?? NourishTheme.Color.primary).opacity(0.15))
                    .frame(width: 48, height: 48)
                Image(systemName: iconFor(supp.name))
                    .font(.system(size: 20))
                    .foregroundStyle(Color(hex: supp.color) ?? NourishTheme.Color.primary)
            }
            ...
            Button {
                ...
                Image(systemName: supp.isLoggedToday ? "checkmark.circle.fill" : "circle")
                    ...
                    .foregroundStyle(supp.isLoggedToday
                                     ? (Color(hex: supp.color) ?? NourishTheme.Color.primary)
                                     : Color.secondary.opacity(0.4))
                    .animation(NourishTheme.Animation.snappy, value: supp.isLoggedToday)
            }
            .buttonStyle(.plain)
        }
        .padding(.horizontal, NourishTheme.Spacing.md)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Image(systemName: "pill.fill")
                .font(.system(size: 48))
                .foregroundStyle(NourishTheme.Color.primary.opacity(0.3))
                        ...
                        HStack(spacing: 8) {
                            ForEach(commonSupps, id: \.self) { s in
                                Button(s) { name = s }
                                    ...
                                    .padding(.vertical, 5)
                                    .background(name == s ? NourishTheme.Color.primary : NourishTheme.Color.surface)
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 8), spacing: 12) {
                        ForEach(colorPresets, id: \.self) { hex in
                            Circle()
                                ...
                                .onTapGesture { withAnimation(NourishTheme.Animation.snappy) { selectedColor = hex } }
                        }
                    }
                    .padding(.vertical, 4)
                }
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 8), spacing: 12) {
                        ForEach(colorPresets, id: \.self) { hex in
                            Circle()
                                ...
                                .onTapGesture { withAnimation(NourishTheme.Animation.snappy) { supplement.color = hex } }
                        }
                    }
                    .padding(.vertical, 4)
                }
