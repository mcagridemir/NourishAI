// NourishAI — MacroDetailView.swift
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            ForEach(MacroType.allCases, id: \.self) { m in
                                Button {
                                    withAnimation(NourishTheme.Animation.snappy) { macro = m }
                                } label: {
                                    HStack(spacing: 5) {
                                        Image(systemName: m.icon).font(.system(size: 11))
                                        Text(m.localizedName).font(NourishTheme.Font.caption(12))
                                    }
                                    .padding(.horizontal, 12).padding(.vertical, 6)
                                    .background(macro == m ? m.color : NourishTheme.Color.surface)
