// NourishAI — FoodDiaryCalendarView.swift
import SwiftUI
            ...
            LazyVGrid(columns: columns, spacing: 4) {
                ...
                ForEach(daysInMonth, id: \.self) { day in
                    ...
                    .onTapGesture {
                        ...
                        withAnimation(NourishTheme.Animation.snappy) { selectedDate = day }
                    }
                }
            }
        }
    ...
    var body: some View {
        VStack(spacing: 3) {
            Text("\(cal.component(.day, from: date))")
                ...
                .foregroundStyle(isSelected ? .white : isToday ? NourishTheme.Color.primary : .primary)
            ...
            if mealCount > 0 {
                HStack(spacing: 2) {
                    ForEach(0..<min(mealCount, 3), id: \.self) { _ in
                        Circle()
                            .fill(isSelected ? .white.opacity(0.8) : NourishTheme.Color.primary)
        ...
        .background(
            RoundedRectangle(cornerRadius: 8)
                .fill(isSelected ? NourishTheme.Color.primary : isToday ? NourishTheme.Color.primaryLight : Color.clear)
        )
