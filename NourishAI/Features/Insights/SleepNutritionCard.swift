// NourishAI — SleepNutritionCard.swift
...
struct SleepNutritionCard: View {
    ...
    private var insight: String {
        switch sleepQuality {
        case .noData:
            return "Connect Apple Health to see how your sleep affects eating habits."
        case .good:
            return "Great sleep! Well-rested nights are linked to better food choices and reduced cravings."
        case .fair:
            return "Slightly under 7 hours. Mild sleep debt can increase appetite by ~15%. Stay mindful of snacking."
        case .poor:
            return "Less than 5 hours detected. Poor sleep raises ghrelin (hunger hormone) — aim for nutrient-dense meals today."
        }
    }
            ...
            if sleepQuality != .noData {
                HStack(spacing: 20) {
                    ...
                    sleepStat(label: "Today cal", value: "\(todayCalories)",
                              icon: "flame.fill", color: NourishTheme.Color.primary)
                }
                ...
                if todayCalories > 0 && sevenDayAvgCalories > 0 {
                    VStack(spacing: 6) {
                        ...
                        comparisonBar(label: "Today", value: todayCalories,
                                      ...
                                      color: todayCalories > sevenDayAvgCalories ? .orange : NourishTheme.Color.primary)
                    }
                }
            }
    ...
    enum SleepQuality: Equatable {
        ...
        var label: String {
            switch self { case .good: return "Good"; case .fair: return "Fair"; case .poor: return "Poor"; case .noData: return "No data" }
        }
        var color: Color {
