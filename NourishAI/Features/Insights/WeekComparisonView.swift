// NourishAI — WeekComparisonView.swift
...
struct WeekComparisonView: View {
    ...
    private var metrics: [(label: String, icon: String, color: Color, this: Double, last: Double, unit: String)] {[
        ("Calories",     "flame.fill",  .orange, avgCal(thisWeek), avgCal(lastWeek), "kcal"),
        ("Protein",      "bolt.fill",   NourishTheme.Color.macro(.protein), avg(thisWeek, \.protein), avg(lastWeek, \.protein), "g"),
        ("Carbs",        "leaf.fill",   NourishTheme.Color.macro(.carbs),   avg(thisWeek, \.carbohydrates), avg(lastWeek, \.carbohydrates), "g"),
        ("Fat",          "drop.fill",   NourishTheme.Color.macro(.fat),     avg(thisWeek, \.fat), avg(lastWeek, \.fat), "g"),
        ("Health score", "heart.fill",  .red,    avgHealthScore(thisWeek), avgHealthScore(lastWeek), "/100"),
        ("Fiber",        "list.bullet.circle.fill", NourishTheme.Color.macro(.fiber), avg(thisWeek, \.fiber), avg(lastWeek, \.fiber), "g"),
    ]}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Spacer()
                HStack(spacing: 12) {
                    legendDot(color: NourishTheme.Color.primary, label: "This week")
    ...
    private func comparisonRow(_ m: (label: String, icon: String, color: Color, this: Double, last: Double, unit: String)) -> some View {
        ...
        return VStack(spacing: 5) {
            HStack {
                ...
                if abs(pct) >= 1 {
                    ...
                    .foregroundStyle(improved ? NourishTheme.Color.primary : .orange)
                }
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 3)
                        ...
                        .animation(NourishTheme.Animation.smooth, value: m.this)
                }
            }
            .frame(height: 6)
        }
