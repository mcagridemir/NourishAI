                            ...
                            Section(day) {
                                ForEach(meals) { meal in
                                    ...
                                    .swipeActions(edge: .leading) {
                                        ...
                                        .tint(NourishTheme.Color.accent)
                                    }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(MealFilter.allCases, id: \.self) { filter in
                    Button {
                        HapticService.selection()
                        withAnimation(NourishTheme.Animation.snappy) { activeFilter = filter }
                    } label: {
                        Text(filter.rawValue)
                            .font(NourishTheme.Font.caption(12))
                            .padding(.horizontal, 12)
                            .padding(.vertical, 6)
                            .background(activeFilter == filter ? NourishTheme.Color.primary : NourishTheme.Color.surface)
