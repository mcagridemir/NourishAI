// NourishAI — LogWeightSheet.swift
import SwiftUI
import SwiftData
        ...
        NavigationStack {
            VStack(spacing: NourishTheme.Spacing.xl) {
                ...
                Button("Save") { save() }
                    ...
                    .padding(.bottom, NourishTheme.Spacing.lg)
            }
    ...
    private var weightDisplay: some View {
        VStack(spacing: 4) {
            Text(String(format: "%.1f", weightKg))
                ...
                .animation(NourishTheme.Animation.smooth, value: weightKg)
            Text("kg")
        ...
        if abs(diff) >= 0.1 {
            HStack(spacing: 6) {
                Image(systemName: diff < 0 ? "arrow.down.circle.fill" : "arrow.up.circle.fill")
                    .foregroundStyle(diff < 0 ? NourishTheme.Color.primary : .orange)
