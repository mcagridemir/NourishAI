...
struct MealHeatmapView: View {
    ...
    private func color(for date: Date) -> Color {
        ...
        case 1:    return SanaTheme.Color.primary.opacity(0.25)
        case 2:    return SanaTheme.Color.primary.opacity(0.50)
        case 3:    return SanaTheme.Color.primary.opacity(0.75)
        default:   return SanaTheme.Color.primary
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Logging streak").font(SanaTheme.Font.headline())
            ...
            // Legend
            HStack(spacing: 4) {
                Text("Less").font(SanaTheme.Font.caption(10)).foregroundStyle(.secondary)
                ForEach([0, 1, 2, 3, 4], id: \.self) { level in
                    let c: Color = level == 0 ? Color(.systemGray5) : SanaTheme.Color.primary.opacity(0.25 * Double(level))
                    RoundedRectangle(cornerRadius: 2).fill(c).frame(width: 12, height: 12)
                }
                Text("More").font(SanaTheme.Font.caption(10)).foregroundStyle(.secondary)
            }
        }
        .padding()
        .nourishCard()
