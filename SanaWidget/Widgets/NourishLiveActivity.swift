...
private struct LockScreenView: View {
    let state: SanaActivityAttributes.ContentState
...
private struct ExpandedView: View {
    let context: ActivityViewContext<SanaActivityAttributes>

    var state: SanaActivityAttributes.ContentState { context.state }
...
struct SanaLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: SanaActivityAttributes.self) { context in
...
#Preview("Lock Screen", as: .content, using: SanaActivityAttributes(userName: "Alex")) {
    SanaLiveActivity()
} contentStates: {
...
#Preview("Dynamic Island Compact", as: .dynamicIsland(.compact), using: SanaActivityAttributes(userName: "Alex")) {
    SanaLiveActivity()
} contentStates: {
    SanaActivityAttributes.ContentState(
        calories: 1240, calorieTarget: 2000,
...
#Preview("Dynamic Island Expanded", as: .dynamicIsland(.expanded), using: SanaActivityAttributes(userName: "Alex")) {
    SanaLiveActivity()
} contentStates: {
    SanaActivityAttributes.ContentState(
        calories: 1240, calorieTarget: 2000,
